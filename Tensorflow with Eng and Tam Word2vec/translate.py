...
def train(buckets):
	...
	with tf.Session() as sess:
		...
		vectors_variable1 = [v for v in tf.trainable_variables() if ENCODER_EMBEDDING_KEY in v.name]
		...
		h5f = h5py.File('embedding/data.h5','r')
		encvectors = h5f['en'][:]
		sess.run([vectors_variable1.initializer], {vectors_variable1.initializer.inputs[1]: encvectors})
		print("   Word2Vec English Encoding Successful!")
	
		vectors_variable2 = [v for v in tf.trainable_variables() if DECODER_EMBEDDING_KEY in v.name]
		...
		decvectors = h5f['ta'][:]
		sess.run([vectors_variable2.initializer], {vectors_variable2.initializer.inputs[1]: decvectors})
		h5f.close()
